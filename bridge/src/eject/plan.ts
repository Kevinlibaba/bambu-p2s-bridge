 ...
 *
 * 前脸靠下有一个相当深的倒角。逐帧回看录像确认：11.2mm 的件是被**倒角的
 * 中部**推走的，根本没够到倒角以上的竖直面。
 *
 * 斜面接触和平面接触不是一回事 —— 它同时给件向前和向下的分力，向下那部分
 * 只是把件压在板上、增加摩擦。这次成了，但不是「竖直墙让件翻不过去」那种
 * 稳当的机制。
 *
 * 由此有一条确定的结论：推的高度要尽量低。前脸下角在喷嘴上方 4.2mm，
 * 推的高度抬高多少，这个角就跟着抬高多少，能咬住件的那一段就短多少。
 * 贴着板走咬得最多，接触点也最靠近件的底部。
 *
 ...
 *
 * 倒角有多高没量过，而这个数决定了整个方案的适用范围：多高的件能碰到
 * 竖直面、多高的只能吃到斜面、多矮的会被斜面直接顶得爬上去。
 * 想扩大适用范围，先把它量出来。
 */
